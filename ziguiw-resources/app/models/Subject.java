...
public class Subject extends JPASupport {
    ...
    public static List<Resource> findBySubjectIdAndLeve(Long subjectId, Integer leve,Integer length) {
        String hql = "select r from Resource r where r.state = ? and r.subject.id = ? and r.grade.leve = ?" +
                " order by r.createTime desc";
        List<Resource> list = Resource.find(hql, Status.OK,subjectId,leve).fetch(1,length);
        return list;
    }

    public static Subject findByName(String name) {
        return find("bySubjectName",name).first();
    }
}
