...
public class TermSetScheduler {
    ...
    public void run() throws Exception {
        try {
            log.info(String.format("begin execute %s!", TermSetScheduler.class.getName()));
            ...
            log.info(String.format("execute %s complete!", TermSetScheduler.class.getName()));
    ...
    private Integer exec(int pageNo) throws SQLException {
        ...
        Integer pageCount;
        log.info(String.format("termSet total record is %s",rectotal));
     ...
     */
    private void saveTermSets(List<Map<String, Object>> list) {
        for (Map<String, Object> map : list) {
            try {
                log.info("map ->" + map);
                rsRpcManager.saveTermSet(new CaseInsensitiveMap(map));
            } catch (Exception e) {
                log.error("save termSet failed!", e);
            }
        }
    }
}
