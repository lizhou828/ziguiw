...
public class GradeScheduler {
    private static final Log log = LogFactory.getLog("grade");

    @Qualifier(value = "rsRpcManager")
    @Autowired
    private RsRpcManager rsRpcManager;

    @Value(value = "${rpc.gradeurl}")
    private String url;
    ...
    public void run() throws Exception {
        try {
            log.info(String.format("begin execute %s!", GradeScheduler.class.getName()));
            ...
            log.info(String.format("execute %s complete!", GradeScheduler.class.getName()));
    ...
    private Integer exec(int pageNo) throws SQLException {
        ...
        Integer pageCount;
        log.info(String.format("grade total record is %s",rectotal));
     ...
     */
    private void saveGrades(List<Map<String, Object>> list) {
        for (Map<String, Object> map : list) {
            try {
                log.info("map ->" + map);
                rsRpcManager.saveGrade(new CaseInsensitiveMap(map));
            } catch (Exception e) {
                log.error("save grade failed!", e);
            }
        }
    }
}
