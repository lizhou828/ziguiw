...
public class ParentScheduler {
    ...
    private static final Log log = LogFactory.getLog("parent");
    @Qualifier(value = "rsRpcManager")
    @Autowired
    private RsRpcManager rsRpcManager;

    @Value(value = "${rpc.parenturl}")
    private String url;
    ...
    public void run() throws Exception {
        try {
            log.info(String.format("begin execute %s!", ParentScheduler.class.getName()));
            Long parentLastModifiedTime = rsRpcManager.findLastModifiedTime("t_PARENTinfo");
            log.info("userLastModifiedTime is:" + parentLastModifiedTime);
            Integer totalPageCount = exec(parentLastModifiedTime, 1);
            if (totalPageCount != null) {
                for (int pageNo = 2; pageNo <= totalPageCount; pageNo++) {
                    exec(parentLastModifiedTime, pageNo);
                    Thread.sleep(1000);
                }
            }
            log.info(String.format("execute %s complete!", ParentScheduler.class.getName()));
     ...
     */
    private void saveParents(List<Map<String, Object>> list) {
        for (Map<String,Object> map : list) {
            try {
                log.info("map ->" + map);
                rsRpcManager.saveParent(new CaseInsensitiveMap(map));
            } catch (Exception e) {
                log.error("save parent failed!", e);
            }
        }
    }
