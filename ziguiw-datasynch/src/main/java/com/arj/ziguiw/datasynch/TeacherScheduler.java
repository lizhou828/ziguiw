...
public class TeacherScheduler {
    ...
    private static final Log log = LogFactory.getLog("teacher");
    @Qualifier(value = "rsRpcManager")
    @Autowired
    private RsRpcManager rsRpcManager;

    @Value(value = "${rpc.teacherurl}")
    private String url;
    ...
    public void run() throws Exception {
        try {
            log.info(String.format("begin execute %s!", TeacherScheduler.class.getName()));
            Long teacherLastModifiedTime = rsRpcManager.findLastModifiedTime("t_teacherinfo");
            log.info("userLastModifiedTime is:" + teacherLastModifiedTime);
            Integer totalPageCount = exec(teacherLastModifiedTime, 1);
            if (totalPageCount != null) {
                for (int pageNo = 2; pageNo <= totalPageCount; pageNo++) {
                    exec(teacherLastModifiedTime, pageNo);
                    Thread.sleep(1000);
                }
            }
            log.info(String.format("execute %s complete!", TeacherScheduler.class.getName()));
    ...
    private Integer exec(Long lastModifiedTime, int pageNo) throws SQLException{
        ...
        Integer pageCount;
        log.info(String.format("teacher total record is %s",rectotal));
    ...
    /**
     * save teachers by collection data
     *
     * @param list the data from remote server
     */
    private void saveTeachers(List<Map<String, Object>> list) throws SQLException {
        for (Map<String,Object> map : list) {
            try {
                log.info("map ->" + map);
                rsRpcManager.saveTeacher(new CaseInsensitiveMap(map));
            } catch (Exception e) {
