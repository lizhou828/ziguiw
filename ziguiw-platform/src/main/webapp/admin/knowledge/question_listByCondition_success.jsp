...
#newTopics table tr td .btn2 {
	...
	<s:if test="questions != null && questions.list != null">
			...
			<s:iterator value="questions.list" var="question">
				<tr>
					...
					<td>
						...
						<s:if test="#question.state == 1">
				待审批
			</s:if>
						<s:elseif test="#question.state == 2">
				已审批
			</s:elseif>
						<s:elseif test="#question.state == -1">
				已删除
			</s:elseif>
						<s:elseif test="#question.state == -2">
				审批未通过
					...
					<td>
						<s:property value="#question.bestAnswerAuthorNick" />
					</td>
					...
					<td>
						...
						<a href="javascript:void(0)"
							onclick="javascript:getAnswers(<s:property value="#question.id"/>,'<%=path%>')">管理答案</a>
					</td>
				</tr>
			</s:iterator>
		</table>
		...
		<s:if test="questions.pageString != null && questions.pageString != ''">
			<div class="fenye a-r">
				${questions.pageString}
			</div>
		</s:if>
	</s:if>
	<s:elseif test="#parameters.opType[0] == 1">
