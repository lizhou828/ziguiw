...
#newTopics table tr td .btn2 {
	...
	<s:if test="debates != null && debates.list != null">
		...
		<table class="table">
			...
			<s:iterator value="debates.list" var="debate">
				<tr>
					...
					<s:else>
					<a
							href="<%=path%>/admin/knowledge/debate_restoreDebate.action?opIds=<s:property value="#debate.id"/>">恢复</a>|
					</s:else>
						<a href="javascript:void(0)"
							onclick="javascript:getDebateOpinions(<s:property value="#debate.id"/>,'<%=path%>')">管理观点</a>
					</td>
				</tr>
			</s:iterator>
		</table>
		...
		<s:if test="debates.pageString != null && debates.pageString != ''">
			<div class="fenye a-r">
				${debates.pageString}
			</div>
		</s:if>
