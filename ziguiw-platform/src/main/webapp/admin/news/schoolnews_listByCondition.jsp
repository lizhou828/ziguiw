...
<div class="right">
    ...
    <p class="map">新闻：校园新闻</p>

    <form action="<%=path%>/admin/news/schoolNews_listByCondition.action">
        <ul class="col-ul ul_li_sp m-t10">
            <li>
                <span>类型</span>
                <s:select list="userDiaryTypeList" name="query.type" listKey="key" listValue="value" theme="simple"
...
<form action="/admin/news/schoolNews_batchAuditPass.action" method="post" id="auditForm">
    <table class="table">
        <colgroup>
        </colgroup>
        <thead>
        ...
        <s:iterator value="pagedVSchoolNews.list" var="vSchoolNews">
            <tr>
                <td>
                    <input type="checkbox" name="schoolNewsIdList" value="${vSchoolNews.id}">
                </td>
                <td>${vSchoolNews.title}</td>
                <td>${vSchoolNews.type}</td>
                <td>
                        ${vSchoolNews.status}
                    <s:if test="#vSchoolNews.status == \"审核失败\""><a
                            href="newsAudit_viewRejectReason.action?schoolNewsId=${vSchoolNews.id}">查看原因</a></s:if>
                    <s:if test="#vSchoolNews.status == \"已经删除\""><a
                                    href="schoolNews_restore.action?id=${vSchoolNews.id}">
                        还原</a></s:if>
                </td>
                <td>${vSchoolNews.user}</td>
                <td>${vSchoolNews.schoolName}</td>
                <td>${vSchoolNews.createTime}</td>
                <td>${vSchoolNews.viewCount}</td>                
                <td>
                    <a href="<%=path%>/admin/news/schoolNews_getById.action?id=${vSchoolNews.id}">修改</a>
                    <a href="<%=path%>/admin/news/schoolNews_deleteById.action?id=${vSchoolNews.id}">删除</a>
                </td>
            </tr>
        </s:iterator>
    </table>
