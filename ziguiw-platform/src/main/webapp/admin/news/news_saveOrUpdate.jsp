...
<form method="post" action="<%=path%>/admin/news/news_saveOrUpdate.action">
<textarea id="content"
                  name="news.content"
                  tabindex="0"
                  style="width:1100;height:400px;margin:0 auto;">${news.content}
    </textarea>
<table class="table">
	<colgroup>
	...
	<tr>
		<td><span>新闻标题</span>
			<input name="news.title" type="text" class="inp_txt" value="${news.title}" maxlength="30"/>
		</td>
		<td class="v-b" ><span class="red_star">*</span>新闻标题，最长不能超过60字</td>
	</tr>
	<tr>
		<td><span>新闻副标题</span>
			<input name="news.subtitle" type="text" class="inp_txt" value="${news.subtitle}" maxlength="30"/>
		</td>
		<td class="v-b" >新闻副标题</td>
	</tr>
	<tr>
		<td><span>关键词</span>
			<input name="news.keywords" type="text" class="inp_txt" value="${news.keywords}" />
		</td>
		<td class="v-b" >关键词，从新闻内容中提炼</td>
	</tr>
	
	<tr>
		<td><span>责任编辑</span>
			<input name="news.editors" type="text" class="inp_txt" value="${news.editors}" disabled="disabled" />
		</td>
	...
	<tr>
		<td><span>新闻首图</span>
			<input name="news.firstImage" type="text" class="inp_txt" value="${news.firstImage}" maxlength="64"/>
		</td>
		<td class="v-b" >新闻的第一张图片，在显示上有特殊处理，如果为空默认用新闻内容的第一张图片</td>
	</tr>
	
	<tr>
		<td><span>新闻PV</span>
			<input name="news.clickCount" type="text" class="inp_txt" value="${news.clickCount}" />
		</td>
		<td class="v-b" >新闻PV</td>
	</tr>
	
	<tr>
		<td><span>描述</span>
			<textarea name="news.description" type="text" class="textarea" value="" cols="10" rows="5">${news.description}</textarea>
		</td>
		<td class="v-b" >新闻描述，如果为空的话自动会从新闻中截取前200字</td>
	</tr>
	
	<tr>
		<td><span>新闻类别</span>
			<select name='news.classType' id="classType" class="inp_select">
	...
	<tr>
		<td><span>创建时间</span>
			<input type="text" class="inp_txt" name="news.createTime" value="${news.formatedStartTime}" onclick="SelectDate(this)" readonly="readonly" />
		</td>
		<td class="v-b" >创建时间</td>
	</tr>
	...
	<tr>
		<td><span>新闻频道</span>
			...
			<select name='news.newsChannle.id' id="channleId" class="inp_select">
		...
		<td class="v-b" ><span class="red_star">*</span>新闻频道</td>
	</tr>
	
	<tr>
		<td><span>新闻省份</span>
			<select name="news.province" id='province' onChange="getCity()" class="inp_select"/>
			<s:if test="news.province!=null"><option value="${news.province}">${news.province}</option></s:if><s:else><option value="">请选择所在省份</option></s:else>
		...
		<td class="v-b" >地方新闻的省份</td>
	</tr>
	
	<tr>
		<td><span>新闻城市</span>
			<select name='news.city' id="city" class="inp_select">
			<s:if test="news.city!=null"><option value="${news.city}">${news.city}</option></s:if><s:else><option selected="selected" value="">请选择所在城市</option></s:else>			    
			</select>
		</td>
		<td class="v-b" >地方新闻的城市</td>
	</tr> 

</table>
<input type="hidden" name="news.state" value="1"/>
 <input type="hidden" name='news.id' value="${news.id}"/>

<p class="submit">
    <input name="Submit1" id="Submit1" type="submit" value="提 交"  class="inp_btn" onclick="getEditorContent()"/>
    <a href="/admin/news/newsAudit_view.action?newsId=${news.id}">审核失败</a>
</p>
</form>
</div>
