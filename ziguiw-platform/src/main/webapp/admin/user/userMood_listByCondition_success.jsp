...
<%@ include file="/admin/inc/navigation.jsp"%>
	...
	<s:if test='#parameters.opType[0] == null || #parameters.opType[0] <= 1'>
		<form
			action="<%=path%>/admin/user/userMood_listByCondition.action">
...
<li><span>推荐位</span><select name="query.regionId" id="regionId" class="inp_select">
	...
	<s:iterator value="#queryAllRecommendRegion.knowledgeRecommendRegions" var="recommendRegion">
		<s:if test='#recommendRegion.type==8'><option <s:if test='#recommendRegion.id==query.regionId'> selected="selected"</s:if> value="<s:property value="#recommendRegion.id"/>"><s:property value="#recommendRegion.id"/>:<s:property value="#recommendRegion.name"/></option></s:if>
	...
	<s:if test="userMoods != null && userMoods.list != null">

		<s:fielderror />
		<s:if test='#parameters.opType[0] == 0'>
		<form name="myform" id="myform"
				action="<%=path%>/admin/user/userMood_delete.action">
		</s:if>
		 
		<s:elseif test='#parameters.opType[0] == 1'>
			<form name="myform" id="myform"
				action="<%=path%>/admin/user/userMood_recommend.action">
		</s:elseif>
		...
		<table class="table">
			...
			<s:iterator value="userMoods.list" var="userMood">
				<tr>
					<td>
						<input type="checkbox" name="opIds"
							value=<s:property value="#userMood.id"/> onclick='unselectall()'>
					</td>
				    <td>
						<s:property value="#userMood.id"/>
					</td>
					<td>
						<s:property value="#userMood.text"/>
					</td>
			
					<td>
						<s:property value="#userMood.knowledgeRecommendRegion.name" />
					</td>
					<td>
					<a href="<%=path%>/admin/user/userMood_delete.action?opIds=<s:property value="#userMood.id"/>">删除</a>
					<!--  
					|
					<s:if test="#userMood.knowledgeRecommendRegion != null && #userMood.knowledgeRecommendRegion.id > 0">
						<a href="<%=path%>/admin/user/userMood_cancleRecommend.action?opIds=<s:property value="#userMood.id"/>">取消推荐</a>
					</s:if>
					<s:else>
					<a href="<%=path%>/admin/user/userMood_recommend.action?opIds=<s:property value="#userMood.id"/>">推荐</a>
					</s:else>
					-->
					</td>
				</tr>
	 ...
	 <s:if test='#parameters.opType[0] == 0'>
	   ...
	   <s:if test='#parameters.opType[0] == 1'>
			...
			推荐至<select name="recommendRegionId" class="inp_select">
				<s:iterator value="#queryAllRecommendRegion.knowledgeRecommendRegions" var="recommendRegion">
				<s:if test='#recommendRegion.type==8'><option value="<s:property value="#recommendRegion.id"/>"><s:property value="#recommendRegion.id"/>:<s:property value="#recommendRegion.name"/></option></s:if>
				</s:iterator>
			</select>
		...
		<s:if test="userMoods.pageString != null && userMoods.pageString != ''">
			<div class="fenye a-r">
				${userMoods.pageString}
			</div>
		</s:if>
	</s:if>
	<s:elseif test="#parameters.opType[0] == 1">
