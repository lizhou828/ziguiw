...
<p class="map">内容管理：留言管理</p>
<form action="<%=path%>/admin/content/message_list.jsp" method="post">
...
<%try{ %>
	<s:action var="action1" name="commonQuery" namespace="/" executeResult="false" ignoreContextParams="true">
			      <s:param name="hql">from UserMessage where kind = 1 
...
<s:param name="queryString"><%if(StringUtils.isNotEmpty(request.getParameter("nickName"))){ %>user.nickName=<s:property value="#parameters.nickName" /><%} %></s:param>
	...
	<tr>
		...
		<td>
			<s:property value="#comment.formatedCreateDate"/>
		</td>
		<td>
		...
		<td>
			<a href="<%=path%>/admin/content/delMessage.action?message.id=<s:property value="#comment.id"/>">删除</a>
		</td>
