...
<p class="map">内容管理：新闻评论</p>
<form action="<%=path%>/admin/content/news_comment.jsp" method="post">
...
<table class="table">
	...
	<s:action var="action1" name="commonQuery" namespace="/" executeResult="false" ignoreContextParams="true">
			      <s:param name="hql">from UserComment where type = 2 and kind = 1 
...
<s:param name="queryString"><%if(StringUtils.isNotEmpty(request.getParameter("nickName"))){ %>user.nickName=<s:property value="#parameters.nickName" /><%} %></s:param>
	...
	<tr>
		...
		<td>
			<a href="<%=path%>/admin/content/delNewsComment.action?message.id=<s:property value="#comment.id"/>">删除</a>
		</td>
