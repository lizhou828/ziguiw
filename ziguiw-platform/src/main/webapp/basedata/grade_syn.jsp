...
GetMethod getMethod = new GetMethod(ConfigUtils.getStringByKey("base_data_syn_url", urlLocation + "/DSIS_zgw_syndata/") + "general.htm?table=grade&pages=" + currentPage + "&pagesize=" + countPerPage);
...
<%	
	try{
		CopyGrade grade = new CopyGrade();
		grade.setNj_id(tempJson.getInt("njId"));
		grade.setNj_mcheng(tempJson.getString("njMcheng"));
//		grade.setNj_bma(tempJson.getString("njBma"));
		grade.setNj_ztai(tempJson.getInt("njZtai"));
		grade.setSffban(tempJson.getInt("sffban"));
//		grade.setNj_xsming(tempJson.getString("njXsming"));
		grade.setXXID(tempJson.getString("xxid"));
		grade.setKqRule(tempJson.getInt("kqrule"));
		
		commonService.saveOrUpdate(grade);
	}catch(Exception e){
...
%>
<script>
window.location.href="grade_syn.jsp?page=" + <%=(Integer.parseInt(currentPage) + 1)%>;
</script>
<%	
}
%>
