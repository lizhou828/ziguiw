...
GetMethod getMethod = new GetMethod(ConfigUtils.getStringByKey("base_data_syn_url", urlLocation + "/DSIS_zgw_syndata/") + "general.htm?table=dept&pages=" + currentPage + "&pagesize=" + countPerPage);
...
<%	
	try{
		CopyDepartment dept = new CopyDepartment();
		dept.setBm_ID(tempJson.getInt("deptId"));
		dept.setBm_mcheng(tempJson.getString("deptName"));
		dept.setPBm_id(tempJson.getInt("pdeptId"));
		dept.setXxID(tempJson.getString("xxid"));
...
%>
<script>
window.location.href="dept_syn.jsp?page=" + <%=(Integer.parseInt(currentPage) + 1)%>;
</script>
<%	
}
%>
