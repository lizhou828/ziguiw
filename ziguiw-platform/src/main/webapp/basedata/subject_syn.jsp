...
GetMethod getMethod = new GetMethod(ConfigUtils.getStringByKey("base_data_syn_url", urlLocation + "/DSIS_zgw_syndata/") + "general.htm?table=subject&pages=" + currentPage + "&pagesize=" + countPerPage);
...
<%	
	try{
		CopySubject subject = new CopySubject();
		subject.setSUBJECT_id(tempJson.getInt("subjectId"));
		subject.setSUBJECT_NAME(tempJson.getString("subjectName"));
		subject.setSUBJECT_CODE(tempJson.getString("subjectCode"));
		subject.setXxID(tempJson.getString("xxid"));
		
		commonService.saveOrUpdate(subject);
	}catch(Exception e){
...
%>
<script>
window.location.href="subject_syn.jsp?page=" + <%=(Integer.parseInt(currentPage) + 1)%>;
</script>
<%	
}
%>
