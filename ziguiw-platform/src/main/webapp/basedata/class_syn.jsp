...
GetMethod getMethod = new GetMethod(ConfigUtils.getStringByKey("base_data_syn_url", urlLocation + "/DSIS_zgw_syndata/") + "general.htm?table=class&pages=" + currentPage + "&pagesize=" + countPerPage);
...
<%	
	try{
		CopyClasz clasz = new CopyClasz();
//		clasz.setBj_bma(tempJson.getInt("bjBma"));
		clasz.setBj_id(tempJson.getInt("bjId"));
		clasz.setBj_mcheng(tempJson.getString("bjMcheng"));
		clasz.setNj_id(tempJson.getString("njid"));
		clasz.setBj_ztai(tempJson.getInt("bjZtai"));
		clasz.setXxID(tempJson.getInt("xxid"));
...
%>
<script>
window.location.href="class_syn.jsp?page=" + <%=(Integer.parseInt(currentPage) + 1)%>;
</script>
<%	
}
%>
