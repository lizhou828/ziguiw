...
<div class="content clearfix pr">
	...
	<div class="w-760 r userCenterContent"	>
		...
		<div class="formBtnBar formBtnBar-bottom" style="margin-right: 0px">
			<a
				href="<%=path%>/user/business/remark_saveOrUpdate.jsp?kind=<%=request.getParameter("kind")%>"><input
				type="button" onclick="window.location.href='<%=path%>/user/business/remark_saveOrUpdate.jsp?kind=<%=request.getParameter("kind")%>';" class="cx" value="增加"></input></a>
		</div>
		<%
			}
		%>
		...
		<s:iterator value="#commonMessages.commonMessages.list" var="message">
			<div class="mes_list_r" style="margin-left: 0px">
				<label class="gray"><span class="r">
						<a
						href="<%=path%>/user/remarkMsgs.action?commonMessageId=<s:property value="#message.id"/>"
						...
						value="#message.leastMsg.formatedCreateDate" /> 评语</label>
				<p class="block">
