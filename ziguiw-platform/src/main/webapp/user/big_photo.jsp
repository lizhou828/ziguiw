...
<div class="mypos1 fsong mb10"> <a href="<%=path%>/user/photo_upload.jsp" class="r btn4">上传图片</a><a href="#">我的小家</a> &gt; <a href="#">我的相册</a> &gt; ${album.title}</div>
	<div class="clearfix">
	<div class="my_home_xc_list pr">
	<a href="<%=path%>/user/getRoundPhoto.aciton?photoId=${prePhoto.id}&albumId=${albumId}&hostUserName=${prePhoto.user.nickName}" class="my_home_list_l"><img alt="" src="<%=path%>/images/my_home_xc_list_l1.png"></a>
	<!--可以点击时，使用
	<a href="#" class="my_home_list_l"><img alt="" src="images/my_home_xc_list_l2.png"></a>-->
		<ul class="img_f">
		<s:iterator value="photos" var="photo">
			<li><a class="a-img1" href="<%=path%>/user/getRoundPhoto.action?photoId=<s:property value='#photo.id'/>&albumId=${albumId}&hostUserName=<s:property value='#photo.user.nickName'/>" ><img src="<%=ImageUtils.getSizedImage((String)((com.opensymphony.xwork2.util.ValueStack)request.getAttribute("struts.valueStack")).findValue("#photo.url"), path + "/images/head.jpg", 70)%>" alt=""></a></li>
		</s:iterator>   
		
       
		</ul>    
	<a href="<%=path%>/user/getRoundPhoto.action?photoId=${nextPhoto.id}&albumId=${albumId}&hostUserName=${nextPhoto.user.nickName}" class="my_home_list_r"><img alt="" src="<%=path%>/images/my_home_xc_list_r1.png"></a>
	<!--可以点击时，使用
	<a href="#" class="my_home_list_l"><img alt="" src="images/my_home_xc_list_r2.png"></a>-->

	</div>
	<div class="bordb xc_dt">
        <img alt=" " src="<s:property value='photo.url'/>" onmouseover="upNext(this)">
	</div>
	</div>
	...
	<div class="bordb my_home hi-at mt10">
		...
		<form action="<%=path%>/user/commentPhoto.action">
	...
	<div class="clearfix message_list pr">
		...
		<div class="mes_list_r">
			<label class="gray"><span class="r"><a href="<%=path%>/user/delPhotoCommentById.action?message.id=<s:property value="#message.id"/>&albumId=${albumId}&message.commentedId=<s:property value='photo.id'/>" class="m-lr4">删除</a> </span> <s:property value="#message.formUser.nickName"/> <s:property value="#message.createTime"/> 留言</label>
			<p class="block"><s:property value="#message.text"/></p>
				...
				<s:iterator value="#message.sortedChildMsgs" var="childMsg">
					...
					<dd class="gray mt5"><s:property value="#childMsg.createTime"/> <a href="<%=path%>/user/delPhotoCommentById.action?message.id=<s:property value="#childMsg.id"/>&albumId=${albumId}&message.commentedId=<s:property value='photo.id'/>">删除</a></dd>
					</dl>
				</s:iterator>
				<form action="<%=path%>/user/commentPhoto.action">
				<p class="mes_hf_k">
...
function upNext(bigimg){  
    ...
    bigimg.onmouseup=function(){  
        top.location= "<%=path%>/user/getRoundPhoto.action?photoId=" + imgurl + "&albumId=${albumId}&hostUserName=<%=hostUser.getNickName()%>";  
    }  
}
</script>
