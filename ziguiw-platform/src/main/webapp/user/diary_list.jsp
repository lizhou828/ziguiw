...
<div class="r w-770   hei-400">
	...
	<div class="my_home">
	<div class="h3titb">
		<h3 class="fyahei">我的日志<%if(isHost){ %><a style="position:absolute;right:15px" class="r" href="<%=path%>/user/publish_diary.jsp">写日志</a><%} %></h3>
	</div>
	<div class="bordb dygs mb10 my_art">
		...
		<s:iterator value="#diaryList.pagedDiaries.list" var="diaries">
			<div class="my_art_dy">
				<h4><a href="<%=path%>/user/getDiaryById.action?diary.id=<s:property value="#diaries.id"/>&hostUserName=${diaries.user.nickName}" target="_blank" class="red_f63"><s:property value="#diaries.title"/></a></h4>
		 		<p><s:property value="#diaries.description" escape="false"/></p>
				<p class="c_666">
				阅读(<s:property value="#diaries.clickCount"/>)
		 		<a href="<%=path%>/user/getDiaryById.action?diary.id=<s:property value="#diaries.id"/>&hostUserName=${diaries.user.nickName}" target="_blank">查看全文</a>
