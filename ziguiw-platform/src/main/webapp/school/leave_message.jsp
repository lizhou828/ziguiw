...
<div class=" mypos1 fsong mb10">  留言板</div>
	...
	<form action="<%=path%>/user/publishSchoolMessage.action" method="post">
	...
	<div class="clearfix message_list pr">
		...
		<div class="mes_list_r">
			<label class="gray"><span class="r"><a href="<%=path%>/user/delSchoolMessageById.action?message.id=<s:property value="#message.id"/>" class="m-lr4">删除</a> </span> <s:property value="#message.formUser.nickName"/> <s:property value="#message.createTime"/> 留言</label>
			<p class="block"><s:property value="#message.text"/></p>
				<div class="mes_hf">
				<s:iterator value="#message.sortedChildMsgs" var="childMsg">
					<dl>
					<dt><a href="<%=path%>/user/portal.jsp?hostUserId=<s:property value="#childMsg.formUser.id"/>" target="_blank" ><img style="float:left;" alt="" src="<s:property value="#childMsg.formUser.portrait"/>" width="48" height="48"></a></dt>
					...
					<dd class="gray mt5"><s:property value="#childMsg.createTime"/> <a href="<%=path%>/user/delSchoolMessageById.action?message.id=<s:property value="#childMsg.id"/>">删除</a></dd>
					</dl>
				</s:iterator>
