...
$('#ProId').change(function(){
	...
	for(var i=0;i<citys.length;i++){
		...
		$('#CityId').append("<option value='"+ $('#ProId').val() + "000" + i +"'>"+citys[i]+"</option>");
	}
});
...
function initSelect(){
	...
	var oldProId = <%=school.getProId()%>
	var oldCityId = <%=school.getCityId()%>
	
	
    for(var i=0;i<ProId.options.length;i++)
    {
        if(ProId.options[i].value == oldProId)
        {
        	ProId.options[i].selected = true;
            break;
        }
    }
    
    $('#CityId').html('');
	var citys=x[oldProId].split(",");
	for(var i=0;i<citys.length;i++){
		
		$('#CityId').append("<option value='"+ oldProId + "000" + i +"'>"+citys[i]+"</option>");
	}
    
    for(var i=0;i<CityId.options.length;i++)
    {
        if(CityId.options[i].value == oldCityId && oldCityId != "")
        {
        	CityId.options[i].selected = true;
        	break;
        }
    }
