...
public class ConvertToSWF {
	 ...
	 */
	public boolean convertFile(String sourcePath,String pdfPath,String swfPath) {
		...
		String realSourcePath = ConfigUtils.getProperty("static.savedPath");
		realSourcePath = realSourcePath.substring(0,realSourcePath.lastIndexOf("/")+1) + sourcePath;
		sourceFile = new File(realSourcePath);
		 
		//创建存放swf文件的文件夹
		String realSwfPath = realpath+swfPath;
		realSwfPath = realSwfPath.substring(0, realSwfPath.lastIndexOf("/"));
		File file = new File(realSwfPath);
		if(!file.exists()){
			file.mkdirs();
		}
		...
		if(!swfFile.exists()){
			if(pdfFile.exists()) {
				try {
					...
					String swfToolsPath = ConfigUtils.getProperty("resources.swfTool");
	 ...
	 * @param ise	
	 */
	public int clearCache(InputStream isi, InputStream ise){
		try {
